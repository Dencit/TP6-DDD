...
class ThinkEx
{
    protected $r="~r"; protected $n="~n"; protected $t="~t"; protected $s="~s";
    protected $opt;
    ...
    function readyToReplace(&$fileContent){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
    ...
    function readyToContent(&$fileContent){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $fileContent = preg_replace("/(".$r.")/is","\r",$fileContent);
    ...
    function fileEditCopy($option,$callfunc){
        ...
        $moduleDomeName = $this->moduleDomeName; $childDemoName = $this->childDemoName;
    ...
    function makeDemoContent(&$content,$option,$opt,$dbOn=null,$dbOptStr=null){
        //修改默认数据库连接配置,用于跨库查询
        if($dbOptStr){ $this->changeDbOption($dbOptStr,$opt); }
        //#
    ...
    function forValidate($childName){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        ...
        if($res){
            foreach ($res as $k=>$v) {
                switch ($v['native_type']){
                    ...
                    $v['native_type']='integer'; break;
                    case "VAR_STRING": case "BLOB":
                    $v['native_type']='alphaDash'; break;
                    case "DATETIME":  case "TIMESTAMP":
                    $v['native_type']='date'; break;
                    case "NEWDECIMAL":
                        $v['native_type']='number'; break;
                };
                switch ($v['name']){
                    default :  break;
                    case "status": $v['native_type']='integer|in:1,2'; break;
                    case "type": $v['native_type']='integer|in:1,2'; break;
                    case "phone": $v['native_type']='number|max:11'; break;
                    case "sex": $v['native_type']='integer|in:0,1,2'; break;
                    case "ip": $v['native_type']='ip'; break;
                };
                if ($k < count($res) - 1) {
    ...
    function forFillable($childName){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $res = $this->DB($childName,"getTableFields");
        $repFillable=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v){
                switch ($v['name']){
                    case "id": case "created_at": case"updated_at": case"deleted_at":
                    break;
                    default:
                        if( $k<count($res)-1 ){
    ...
    function forGuarded($childName){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $res = $this->DB($childName,"getTableFields");
        $repFillable=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v){
                switch ($v['name']){
                    case "id": case "create_time": case"update_time":
                    if( $k<count($res)-1 ){
                        $repFillable .= "\"".$v['name']."\",".$space;
                    }else{
                        $repFillable .= "\"".$v['name']."\",".$space;
    ...
    function forTypes($childName){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $res = $this->DB($childName,"getTableFields");
        $repFillable=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v) {
                ...
                switch ($v['native_type']){
                    ...
                    case "LONGLONG":
                        $v['native_type']='bigint'; break;
                    case"TINY":
                        $v['native_type']='tinyint'; break;
                    case "VAR_STRING": case "BLOB":
                    $v['native_type']='varchar'; break;
                    case "DATETIME":  case "TIMESTAMP":
                    $v['native_type']='datetime'; break;
                    case "NEWDECIMAL":
                        $v['native_type']='numeric'; break;
                };
                switch ($v['name']){
                    default :  break;
                    case "status": $v['native_type']='tinyint'; break;
                    case "type": $v['native_type']='tinyint'; break;
                };
                if ($k < count($res) - 1) {
    ...
    function forRules($childName){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $res = $this->DB($childName,"getTableFields");
        $repFillable=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v) {
    ...
    function forData($childName){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $res = $this->DB($childName,"getTableFields");
        $repFillable=''; $space=$r.$n.$t.$t.$t;
        if($res){
            foreach ($res as $k=>$v) {
                //var_dump( $v['native_type'] );
                switch ($v['native_type']){
...
//#########################################################
    ...
    function setDocFile($moduleName,$childName,$dbOn=null){
        ...
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetDocFile | is exists !"; $this->console($msg,"red"); }
        else {
    ...
    function setControllerFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        $moduleDomeName = $this->moduleDomeName; $childDemoName = $this->childDemoName; $opt=$this->opt;
        $domainPath = $this->opt['domain_path'];
        $option=[
        ...
        if($isExistFile){
            ...
            $msg = "Changed : Module ".$moduleName.' | SetControllerFile | OK'; $this->console($msg , "green");
            ...
            $msg="Created : Module ".$moduleName.' | SetControllerFile | OK'; $this->console($msg,"yellow");
            return true;
        }
        return false;
    }
    function setCmdFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetCmdFile | is exists !"; $this->console($msg,"red"); }
        else{
            $this->fileEditCopy($option,function($option,$content)use($opt,$dbOn){
                //替换指定文本
                $ContentReplace = new ContentReplace($option,$opt); $ContentReplace ->forNameNormal($content);
                //替换注释标签
                $ContentReplace->forHiddenTag($content);
                return $content;
            });
            $msg="Created : Module ".$moduleName.' | SetCmdFile | OK'; $this->console($msg,"yellow");
            return true;
        }
        return false;
    }
    function setSaveJobFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | setSaveJobFile | is exists !"; $this->console($msg,"red"); }
        else{
            $this->fileEditCopy($option,function($option,$content)use($opt,$dbOn){
                //替换指定文本
                $ContentReplace = new ContentReplace($option,$opt); $ContentReplace ->forNameHump($content);
                //替换注释标签
                $ContentReplace->forHiddenTag($content);
                return $content;
            });
    ...
    function setRequestFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetRequestFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) use($opt,$dbOn,$dbOptStr) {
                //修改默认数据库连接配置,用于跨库查询
                if($dbOptStr){ $this->changeDbOption($dbOptStr,$opt); }
    ...
    function setRepositoryFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | setRepositoryFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) use($opt,$dbOn,$dbOptStr) {
                //修改默认数据库连接配置,用于跨库查询
                if($dbOptStr){ $this->changeDbOption($dbOptStr, $opt); }
    ...
    function setModelFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetModelFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) use($opt,$dbOn,$dbOptStr) {
                //修改默认数据库连接配置,用于跨库查询
                if($dbOptStr){ $this->changeDbOption($dbOptStr,$opt); }
    ...
    function setTransFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetTransFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) use($opt,$dbOn,$dbOptStr) {
                //修改默认数据库连接配置,用于跨库查询
                if($dbOptStr){ $this->changeDbOption($dbOptStr,$opt); }
    ...
    function setSrvFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        if($isExistFile){
            ...
            $msg = "Changed : Module ".$moduleName.' | SetSrvFile | OK'; $this->console($msg , "green");
            ...
            $msg = "Created : Module ".$moduleName.' | SetSrvFile | OK'; $this->console($msg , "yellow");
            return true;
        }
        return false;
    }
    function setLogicFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        if($isExistFile){
            ...
            $msg = "Changed : Module ".$moduleName.' | SetLogicFile | OK'; $this->console($msg , "green");
            ...
            $msg = "Created : Module ".$moduleName.' | SetLogicFile | OK'; $this->console($msg , "yellow");
            return true;
        }
        return false;
    }
    function setEnumFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetEnumFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) {
                //替换指定文本
                $ContentReplace = new ContentReplace($option); $ContentReplace ->forNameLower($content);
    ...
    function setErrorFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetRootErrorFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) {
                //替换指定文本
                $ContentReplace = new ContentReplace($option); $ContentReplace ->forNameNormal($content);
                return $content;
            });
    ...
    function setAttributeFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetAttributeFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) {
                //替换指定文本
                $ContentReplace = new ContentReplace($option); $ContentReplace ->forNameNormal($content);
    ...
    function setMessageFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetMessageFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) {
                //替换指定文本
                $ContentReplace = new ContentReplace($option); $ContentReplace ->forNameLower($content);
    ...
    function setRoute($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetRouteFile | route is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) {
                //替换指定文本
    ...
    function makeFolder ($moduleName,$message=null){
        ...
        foreach ($childPathArr as $k=>$v){
            ...
            if($isExistFile){
                if($message){
                    $msg="Exception : Module ".$moduleName." | MakeFolder | ".$v." is exists !"; $this->console($msg,"red");
                }
            }
            else{
                ...
                if($res){
                    if($message){
                        $msg="Created : Module ".$moduleName." | MakeFolder | make ".$v." folder OK"; $this->console($msg,"yellow");
                    }
                }
            }
            usleep(100);
    ...
    function makeTestFolder($moduleName,$message=null){
        ...
        foreach ($childPathArr as $k=>$v){
            ...
            if($isExistFile){
                if($message){
                    $msg="Exception : TestModule ".$moduleName." | MakeFolder | ".$v." is exists !"; $this->console($msg,"red");
                }
            }
            else{
                $res = mkdir(iconv("UTF-8", "GBK", $childPath ),0755,true);
                if($res){
                    if($message){
                        $msg="Created : TestModule ".$moduleName." | MakeFolder | make ".$v." folder OK"; $this->console($msg,"yellow");
                    }
                }
            }
            usleep(100);
    ...
    function setTestFile($moduleName,$childName,$dbOn=null,$dbOptStr=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetUrlTestFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) use($opt,$dbOn,$dbOptStr) {
                //修改默认数据库连接配置,用于跨库查询
                if($dbOptStr){ $this->changeDbOption($dbOptStr,$opt); }
    ...
    function setTestBehaviorFile($moduleName,$childName,$dbOn=null){
        ...
        $isExistFile = file_exists($option['put_path']);
        if($isExistFile){ $msg="Exception : ".$moduleName." | SetTestFile | is exists !"; $this->console($msg,"red"); }
        else {
            $this->fileEditCopy($option , function ($option , $content) use($opt,$dbOn) {
                $ContentReplace = new ContentReplace($option,$opt);
                //替换指定文本
