...
class ContentReplace
{
    ...
    public function forNameNormal( &$content ){
        $moduleName = $this->moduleName; $childName = $this->childName;
        $moduleDemoName = $this->moduleDemoName; $childDemoName = $this->childDemoName;
    ...
    public function forHiddenTag( &$content ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;  $space=$r.$n.$s.$s.$s.$s;
        $start=''; $end='';
        if( !$this->opt['debug'] ){
            $start="/*"; $end="*/";
    ...
    public function forValidate(  &$content  ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName); $repValidate=''; $space=$r . $n . $t . $t;
            ...
            foreach ($res as $k=>$v) {
                ...
                switch ($typeArr[0]){
                    ...
                    case "decimal":
                        ...
                        break;
                    case "string": $ruleStr='alphaDash'.'|length:0,'.$typeArr[1];
                        break;
                    case "text": $ruleStr='chsDash'; break;
                ...
                switch ( $field ){
                    ...
                    case "date": $ruleStr='dateFormat:Y-m-d'; break;
    ...
    public function forMessage(  &$content  ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName); $repValidate=''; $space=$r . $n . $t . $t;
        $res = $this->DB($childName,"showFullColumns");
        if($res){
            ...
            foreach ($res as $k=>$v) {
                ...
                //处理 不能null的字段
                if( $nullAble=='no' ){ $repValidate .= "'".$field.".require'=>'".$comment." 不能为空'," . $space; }
    ...
    public function forTypes( &$content ){  $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repFillable=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v) {
                ...
                switch ($v['native_type']){
                    ...
                    case "STRING": case "VAR_STRING": case "BLOB":
                    $v['native_type']='varchar'; break;
                    case "DATE": case "DATETIME":  case "TIMESTAMP":
                        //其它时间字段 忽略转换
                        $inFields = ['create_time','update_time','delete_time'];
                        if( !in_array($v['name'],$inFields) ){ $v['native_type']=''; }
                        else{ $v['native_type']='datetime'; }
                    break;
                    case "NEWDECIMAL":
            ...
            foreach ($res as $k=>$v) {
                ...
                switch ($v['name']) {
                    default:
                        $repFillable .= "\"".$v['name']."\"=>".$v['native_type']."," .$space;
                        break;
                    case "mobile": $repFillable .= "\"".$v['name']."\"=> '18500000000' ," .$space; break;
                    case "photo": $repFillable .= "\"".$v['name']."\"=> '/none',".$space; break;
            ...
            foreach ($res as $k=>$v) {
                ...
                switch ($v['name']) {
                    default:
                        ...
                        break;
                    case "mobile": $repFillable .= "\"".$v['name']."\"=> '18500000000' ," ; break;
                    case "photo": $repFillable .= "\"".$v['name']."\"=> '/none'," ; break;
                    case "create_time": $repFillable .= "\"".$v['name']."\"=> \$createTime," ; break;
                    case "update_time": case "delete_time": break;
                }
