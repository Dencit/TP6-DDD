...
use extend\thinkex\base\exception\BaseErrorCode;
use extend\thinkex\base\exception\Exception;
...
class ExcelTable
{
    ...
    public function getUpLoadExcel(){
        ...
        if ($file == null) {
            Exception::app(BaseErrorCode::code('UPLOAD_FILE_NOT_FOUND'),BaseErrorCode::msg('UPLOAD_FILE_NOT_FOUND'));
        }
        $temp = explode(".", $_FILES["file"]["name"]);
        $extension = end($temp);
        if( !in_array( $extension, ["xlsx","xls"] ) ){
            Exception::app(BaseErrorCode::code('UPLOAD_FILE_NOT_EXCEL'),BaseErrorCode::msg('UPLOAD_FILE_NOT_EXCEL'));
        }
    ...
    public function excelFileImport($fileData){
        ...
        if ( !$reader->canRead($fileData) ) {
            ...
            if (!$reader->canRead($fileData)) {
                Exception::app(BaseErrorCode::code('UPLOAD_EXCEL_ONLY'),BaseErrorCode::msg('UPLOAD_EXCEL_ONLY'));
            }
        }
